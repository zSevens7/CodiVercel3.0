...
// --- DEFINIÇÕES DE TIPO ---
// (Adicione ou ajuste conforme a estrutura exata do seu projeto)

export interface Gasto {
  data: Date | string;
  preco: number; // Espera-se que seja um valor negativo, ex: -150.00
  // ...outras propriedades do gasto
}

export interface Venda {
  data: Date | string;
  // ✅ CORREÇÃO AQUI: Permitir que valorFinal seja 'number' ou 'null'
  valorFinal: number | null; // Espera-se que seja um valor positivo, ex: 500.00
...
export const aggregateCashFlows = (
  ...
  vendas.forEach(v => {
    ...
    // A lógica 'v.valorFinal ?? 0' já lida com 'null' ou 'undefined'
    const valor = v.valorFinal ?? 0; 
  ...
  gastos.forEach(g => {
    ...
    const preco = g.preco ?? 0; // O valor de 'preco' já é negativo
